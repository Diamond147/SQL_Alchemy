from sqlalchemy import String, ForeignKey, DateTime, UniqueConstraint, Table, Column, Enum
...
class AdminsDepartment(enum.Enum):
  sales = "sales"
  accounts = "accounts"
  logistics = "logistics"
...
class Customer(Base):
    ...
    customeraddress: Mapped[List["CustomerAddress"]] = relationship(back_populates="customer")

    def __repr__(self) -> str:
        return [
            f"Customer(id={self.id!r}, first_name={self.first_name!r}, last_name={self.last_name_name!r},"
            f"username={self.username!r}, email={self.email!r}, phone_number={self.phone_number!r},"
...
class Vendor(Base):
    ...
    def __repr__(self) -> str:
        return [f"Vendor(id={self.id!r}, first_name={self.first_name!r}, last_name={self.last_name!r},"
                f"username={self.username!r}, email={self.email!r}, phone_number={self.phone_number!r},"
                f"address={self.address!r}, password_hash={self.password_hash!r}, date_created={self.date_created!r})"
                ]
...
class Admin(Base):
    ...
    email:Mapped[str] = mapped_column(String(30), unique=True, nullable=False)
    phone_number:Mapped[str] =mapped_column(String(30), unique=True, nullable=False)
    department:Mapped[str] = mapped_column(Enum(AdminsDepartment))
...
class Order(Base):
    ...
    def __repr__(self) -> str:
        return [f"Order(id={self.id!r}, status={self.status!r}, customer_id={self.customer_id!r},"
                f"price={self.price!r}, created_at={self.created_at!r}, updated_at={self.updated_at!r})"]  
...
class OrderItem(Base):
    ...
    __table_args__ = (UniqueConstraint("product_id"),) #This is used in 1-1 relationship
    
    def __repr__(self) -> str:
        return [f"OrderItem(id={self.id!r}, order_id={self.order_id!r}, product_id={self.product_id!r},"
                f"quantity={self.quantity!r}, unit_price={self.unit_price!r}, total_amt={self.total_amt!r})"]    
...
class Product(Base):
    ...
    promotion: Mapped[List["Promotion"]] = relationship(secondary=product_promotion, back_populates="product")
...
class Payment(Base):
    ...
    order_id: Mapped[int] = mapped_column(ForeignKey("orders.id"))
    amount:Mapped[float]
    status:Mapped[str] = mapped_column(Enum(PaymentStatus))
...
class CustomerAddress(Base):
    ...
    customer: Mapped["Customer"] = relationship(back_populates="customeraddress")
...
class Inventory(Base):
    ...
    def __repr__(self) -> str:
        return [f"Inventory(id{self.id!r}, product_id{self.product_id!r}, quantity{self.quantity!r}, last_updated{self.last_updated!r})"]
...
class Review(Base):
    ...
    def __repr__(self) -> str:
        return [f"Review(id{self.id!r}, product_id{self.product_id!r}, customer_id{self.customer_id!r},"
                f"comment{self.comment!r}, rating{self.rating!r}, created_at{self.created_at!r})"]
...
class Promotion(Base):
    ...
    product: Mapped[List["Product"]] = relationship(secondary=product_promotion, back_populates="promotion")

    def __repr__(self) ->  str:
        return [f"Promotion(id{self.id!r}, code{self.code!r}, product_id{self.product_id!r}, discount_percentage{self.discount_percentage!r}," 
                f"description{self.description!r}, start_date{self.start_date!r}, end_date{self.end_date!r} )"]
